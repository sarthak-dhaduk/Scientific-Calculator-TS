import {
    getCrudData,
    setCrudData,
} from "./LockeStorag"; //Import GetData And SetData Function From LockeStorag.ts File.

// Useing in convert value radians to degrees
let dataOfMemory = "" as string | null;
dataOfMemory = getCrudData();
//

let display: string = "";
let subDisplay: number = 0;

let errMsg = document.getElementById("err-msg") as HTMLElement | null;
let displayOT = document.getElementById("input-field") as HTMLElement | null;
let angleShow = document.getElementById("angle") as HTMLButtonElement;
let Special1 = document.getElementById("2ndSpecial-1") as HTMLButtonElement;
let x2 = document.getElementById("x2") as HTMLButtonElement;
let sqrt = document.getElementById("sqrt") as HTMLButtonElement;
let xry = document.getElementById("**") as HTMLButtonElement;
let x10 = document.getElementById("10x") as HTMLButtonElement;
let log = document.getElementById("log") as HTMLButtonElement;
let inn = document.getElementById("in") as HTMLButtonElement;

let sin = document.getElementById("sin") as HTMLButtonElement;
let cos = document.getElementById("cos") as HTMLButtonElement;
let tan = document.getElementById("tan") as HTMLButtonElement;
let cosec = document.getElementById("cosec") as HTMLButtonElement;
let sec = document.getElementById("sec") as HTMLButtonElement;
let cot = document.getElementById("cot") as HTMLButtonElement;

let trigonometryCaseSecond: string = "o";
let trigonometryHyp: string = "o";

let temporaryMemory: string | null = "";

let i: number = 1;
let fact: number = 1;

let memoryRecolButton = document.getElementById("mr") as HTMLButtonElement;
let memoryClearButton = document.getElementById("mc") as HTMLButtonElement;

//Convert value radians to degrees
function radians_to_degrees(radians: string) {
...
function degrees_to_radians(degrees: string) {
    subDisplay = Number(degrees);
    var pi = Math.PI;
    return String(subDisplay * (pi / 180));
}

let buttons: NodeListOf<HTMLButtonElement> = document.querySelectorAll(".jsrun");


Array.from(buttons).forEach((button: HTMLButtonElement) => { //Make an array of all buttons element and apply forEach Loop on it to get data
    button.addEventListener('click', (e) => {

        switch ((e.target as HTMLInputElement).id || (e.target as HTMLInputElement).title) { //getting id and title of targeted button
            case "Eql": // while click equal to (=)
                if (display.includes("rt")) {
                    ...
                    } catch (err: unknown) { //Throw an error and remove it after 3s.
            ...
            case "mc": // while click MC button
                temporaryMemory = "";
                setCrudData(temporaryMemory);
                display = String(getCrudData());
                displayOT!.textContent = display;
            ...
            case "dms": // while click →dms button
                try {
                    subDisplay = Number(display);
                    let d: number = Math.floor(subDisplay);
                    let minfloat: number = (subDisplay - d) * 60;
            ...
            case "angle": //While Click ["DEG" , "RAD" , "GRAD"] it will change the button's value.
                if ((e.target as HTMLInputElement).value == "deg") {
                    angleShow.innerHTML = "RAD";
                    angleShow.value = "rad";
                } else if ((e.target as HTMLInputElement).value == "rad") {
                    angleShow.innerHTML = "GRAD";
                    angleShow.value = "grad";
                } else if ((e.target as HTMLInputElement).value == "grad") {
            ...
            case "2nd": //Any Case Click hyp and 2nd button it will change the trigonometry button's value.
                if (trigonometryHyp == "o" && trigonometryCaseSecond == "o") {
                    ...
                    cot.innerHTML = "cotθ";
                    cot.title = "cot";
                    if ((e.target as HTMLInputElement).id == "2nd") {
                        trigonometryHyp = "z";
                    } else if ((e.target as HTMLInputElement).id == "hyp") {
            ...
            default: //While clicking any button it will display it value on screen
                display += (e.target as HTMLInputElement).id;
                displayOT!.textContent = display;
                break;
        }
        if (getCrudData() != "") { //if loclestorage was null then MR & MC will disabled
